...
- [~] **M13-MEAS-03 Automate the owner-session legs** [detail] (2026-08-07)
  ...
  Status 2026-08-08: automation shipped and gate-proven (D129) — `npm run bench:auto`, sheet shrunk, variant decision-logged, D71 answered in mechanism (lazy major GC, not retention). The quiet-desktop precondition is itself automated (D130): `-- --when-quiet` arms an idle-gated, display-held, environmental-retry run — armed in the background 2026-08-08. Remaining: that armed run landing its valid capture artefact, then the owner Part-A′ cross-check before capture rows are canon.
