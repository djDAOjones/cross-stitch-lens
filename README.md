...
[`pm_skills/project/backlog.md`](pm_skills/project/backlog.md). M0
(scaffold + quality gate) has not yet landed, so the source tree and the
`npm run dev`/`check` pipeline described below are the intended shape,
not yet all present.
