...
- [x] 2026-08-23 DEV-INFRASTRUCTURE.md § Maintainer diagnostics — the email route is "Report a problem": one click saves the settings document (`.json`, never the picture) and the redacted log, then opens a prefilled mailto addressed to `DEV_EMAIL` (a dedicated alias the owner sets); routes now Report · Copy · Download (source: DIAG-02 close; applied by the D186 doc-sync before the handoff arrived)
