...
- The file-map generator maps untracked files, and `check-docs` then fails in every clean checkout: `gen-file-map.mjs` discovers `git ls-files --others --exclude-standard`, so a local-only file (`.codex/hooks.json`) got a mapped line and reddened CI while the local gate stayed green. Including uncommitted files is deliberate — you can map a file before committing it — but nothing warns that mapping one you never intend to commit is a gate trap. Options: have the generator mark untracked entries, or have `check-docs` name the cause. Framework-class change (from: BATCH-E0 CI, D209).
