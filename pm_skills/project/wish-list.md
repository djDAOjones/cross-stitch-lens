...
- Catalogue rows whose name and colour disagree: `ariadna:1650` is `#beffff`, a cyan-white, named "heather very light" (heather is a mauve) — surfaced by Winter frost selecting it. Owner data and protected, so it needs an owner decision, not a fix; rule-shaped profiles reading all brands will keep surfacing rows like it (from: 2026-08-09 M15-GALLERY-01 review)
- Catalogue row with no name at all: `finca:4368` (`#2d6153`) has an empty name field, so it renders in the colour key as a reference with nothing beside it — surfaced by Rainforest selecting it. Same shape as the `ariadna:1650` row: owner data, protected, needs an owner call (from: 2026-08-09 M15-GALLERY-01 batch 2)
- Profile-editor photo slots have no images: `public/profile-demo/` holds only its README, so three of the five test-preview slots render "Image offline" and the M15-GALLERY-01 evidence format cannot include them until the owner adds `landscape.png`, `cartoon.png`, `portrait.png`, `text.png` (from: 2026-08-09 M15-GALLERY-01 review)
