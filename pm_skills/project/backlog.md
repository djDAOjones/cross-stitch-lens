...
- [~] **ADJUST-01 Adjustments 2a: black/white point, curve and saturation as the third profile kind** [detail] (2026-08-23)
  ...
  Status: build landed and verified 2026-08-24 (D202) — the stage, the third profile kind, schema v13 and the adjusted selection source all work in the app; the LUT fingerprint is proved untouched and the frame holds ~7–8 updates/s at the capture size the promise binds at. Open is the owner half: signing the starter set's membership and names (the nine ship as working labels), plus a keyboard pass on the curve points (the driven browser cannot prove native activation).
